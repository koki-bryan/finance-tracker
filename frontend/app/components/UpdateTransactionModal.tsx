...
const UpdateTransactionModal = ({ transaction, onClose }: UpdateTransactionModalProps) => {
  const { fetchTransactions } = useTransactions();
  
  const initialCategory = CATEGORIES.find((c) => c.id === transaction.category_id);
  const initialType = initialCategory?.type || "expense";

  const [modalTransactionType, setModalTransactionType] = useState<"income" | "expense" | null>(initialType);
  const [type, setType] = useState(initialType);
  const [category, setCategory] = useState(transaction.category_id.toString());
